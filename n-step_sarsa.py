...
import pandas as pd
def e_greedy(Q,state, epsilon=0.1):
    while True:
        p = np.random.random()
        all_actions = [(i, j) for i in range(16) for j in range(16)]
        idx_to_action = {idx: a for idx, a in enumerate(all_actions)} 
        action = np.argmax(Q[state]) if p > epsilon else np.random.choice(np.arange(len(Q[state])))
        if idx_to_action[action] in env.get_legal_moves():
            break
    return action
...
def n_step_sarsa(env, num_episodes,n, discount_factor=1.0, alpha=0.5, epsilon=0.1):
    """
  ...
  # → un singolo float
    ...
    # Episode rewards should give the total reward per episode (I use episode rewards to store the rewards we get for each episode and use them for the n-step update)
    episode_rewards = np.zeros((num_episodes,100000)) 
    ...
    for i in tqdm(range(num_episodes)):
        ...
        t=-1
        #let's inizialize an action based on the greedy policy
        action_idx = e_greedy(Q, state)
        ...
        while True:
            ...
            if t<T:
                ...
                if done: # if terminal, the next iteration will be the last one (we no longer enter this branch)
                    ...
                    next_action_idx = e_greedy(Q, next_state)
            ...
            if tau>=0:
                ...
                G = sum(episode_rewards[i][j] * discount_factor**(j-tau-1) for j in range(tau+1,min(tau+n,T)+1))
...
env=RoyalGameOfUr(2)
Q,episode_rewards,episode_lengths = n_step_sarsa(env,1000,2)

# Plot the episode length over time
fig1 = plt.figure(figsize=(10,5))
plt.plot(episode_lengths, label='n-step SARSA')
plt.xlabel("Episode")
...
plt.plot(np.cumsum(episode_lengths), np.arange(len(episode_lengths)), label='n-step SARSA')
